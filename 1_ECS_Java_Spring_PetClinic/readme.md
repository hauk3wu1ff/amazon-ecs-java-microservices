...
The latest AMI image optimized for the Elastic Container Service seems to be always available through  [here](https://aws.amazon.com/marketplace/pp/B06XS8WHGJ?qid=1522499681480&sr=0-1&ref_=srh_res_product_title) on the AWS Marketplace. Also see [ECS documentation](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/ecs-optimized_AMI.html). The market place page also lists the per-hour pricing for running the AMI on different instance types in different regions. This example uses 2 m4.large instances for running the container hosts. See [EC2 instance Types](https://aws.amazon.com/ec2/instance-types/) for instance charcteristics.  